...
Item::Item(int posX, int posY, vector<Bola *> *bolas, double *coefAtritoMesa, bool *miraLouca){
     this->posX = posX;
     this->posY = posY;
     this->bolas = bolas;
     this->coefAtritoMesa = coefAtritoMesa;
     this->miraLouca = miraLouca;
     this->velocidade = Vetor(2,2);
     }
...
void Item::mover()
{
    this->posX += this->velocidade.x;
    this->posY += this->velocidade.y;
}

void Item::setVel(Vetor vel)
{
    this->velocidade = vel;
}
